...
with st.spinner("Loading model... (this happens only once)"):
    ensemble_model, scaler, test_acc, cm, class_counts, imbalance_ratio = train_model()
...
with col2:
    st.info(f"📊 Normal: {class_counts.get(0, 0)}")
with col3:
    st.info(f"📊 C-section: {class_counts.get(1, 0)}")

st.warning(f"⚖️ Class Imbalance Ratio: {imbalance_ratio:.1f}:1 (Model adjusted to handle imbalance)")
